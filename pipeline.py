...
class RenderingPipeline:
    ...
    """
    funzione che avvia la pipeline: crea il frame buffer, disegna il fondale, 
    ...
    def esegui(self, output_path: str):
        ...
        frame_buffer = np.zeros((self.FB_HEIGHT, self.FB_WIDTH), dtype=np.uint8)
    ...
    def _disegna_fondale(self, blitter):
        ...
        for riga in range(len(self.scena.tile_map)):
            for colonna in range(len(self.scena.tile_map[riga])):
                ...
                blitter.disegna_tile(tile_id, dest_x, dest_y)
    ...
    output: no, modifica il frame buffer tramite il blitter
    """
    def _disegna_sprites(self, blitter):
    ...
    def _salva_png(self, frame_buffer, output_path):
        ...
        # crea una matrice RGB vuota: 480 righe, 640 colonne, 3 canali (R, G, B)
        immagine_rgb = np.zeros((self.FB_HEIGHT, self.FB_WIDTH, 3), dtype=np.uint8)
